...
static void renderComboBoxEdit(HWND hWnd, HDC hdc, dmlib_subclass::ComboBoxData& comboBoxData, int iStateID) noexcept
{
	...
	// leftover white from the unpainted client.
...
// dark track + thumb over the non-client strip.
...
/**
 * @brief Window subclass procedure for a combo box drop-down list (ComboLBox).
 *
 * Items are unthemed so `WM_CTLCOLORLISTBOX` applies. `GetSysColor` is hooked
 * while painting so `COLOR_WINDOW` / `COLOR_WINDOWTEXT` match the palette.
 * The classic non-client scrollbar is painted in `WM_NCPAINT`.
 *
 * @param[in]   hWnd        Window handle being subclassed.
 * @param[in]   uMsg        Message identifier.
 * @param[in]   wParam      Message-specific data.
